...
%Compared to deep dense networks which learn a linear combination of the features at each step, each step in a decision tree introducs a splitting criteria based on a feature in the input vector. Tree ensembles 
...
%We suspect that beyond empirically performing better in our case, tree ensembles may be a more generalizable alternative to deep regression networks for this task. After collecting our dataset we visualized each IR file after performing PCA with $PC=7$ and taking the two components with the most explained variance in figure \ref{fig:pca-no-main}. There is no obvious pattern to the distribution of samples, which explains why we observed that the deep regression network failed to learn (see figure \ref{fig:mlp-training}).
